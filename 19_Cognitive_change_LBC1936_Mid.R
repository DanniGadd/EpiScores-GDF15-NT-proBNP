# loc /Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_LBC_brain/
#### library ####
library("tidyverse")
library("lavaan")
library("readr")
...
for(i in episcores) { 
  ...
  reg_Ig <- '
 Ig ~ tmp + ageyearsw1 + sex + smokingScore + bmi_w1 + depind_w1 + alcunitwk_w1
'
...
write.csv(results, file = "/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/04_LBC/SEM_results/LBC_brain/General_Cog_ability_Score_intercept_LBC1936_assocs_2_EpiScores_mid_15122022.csv", row.names = FALSE)
...
for(i in episcores) { 
  ...
  reg_Sg <- '
 Sg ~ tmp + ageyearsw1 + sex + smokingScore + bmi_w1 + depind_w1 + alcunitwk_w1
'
...
write.csv(results, file = "/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/04_LBC/SEM_results/LBC_brain/General_Cog_change(slope)_LBC1936_assocs_2_EpiScores_mid_15122022.csv", row.names = FALSE)
