...
### COX MODELLING FOR GS BIOMARKERS - comparison EpiScores vs proteins
...
# list <- c("smokingScore", "units", "usual", "bmi", "rank", "years")
# for(i in list){ 
...
W4_BNP <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W4_BNP_pQTL.csv")
W4_GDF <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W4_GDF15_pQTL.csv")

names(W4_GDF)[2] <- "gdf15_rnk"
names(W4_BNP)[2] <- "nt.probnp_rnk"

target <- readRDS("/Cluster_Filespace/Marioni_Group/GS/GS_methylation/GS20k/GS20k_Targets.rds")
d1 <- left_join(d1, target, by = "Sample_Name")


d1 <- merge(d1, W4_GDF, by = "Sample_Sentrix_ID", all = T)
table(is.na(d1$gdf15_rnk)) # 2622 samples - correct 

d1 <- merge(d1, W4_BNP, by = "Sample_Sentrix_ID", all = T)
table(is.na(d1$nt.probnp_rnk)) # 2476 samples - correct 

# Add in protein EpiScores as predictors and exclude relevant individuals 
GDF_score <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_scores/projections_GS_GDF_scores_W4_EPIC_10cv.csv")
BNP_score <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_scores/projections_GS_BNP_scores_W4_EPIC_10cv.csv")

GDF_score <- GDF_score[c(1,3)]
BNP_score <- BNP_score[c(1,3)]

names(GDF_score) <- c("GDF_score", "Sample_Sentrix_ID")
names(BNP_score) <- c("BNP_score", "Sample_Sentrix_ID")

d1 <- left_join(d1, GDF_score, by = "Sample_Sentrix_ID")
d1 <- left_join(d1, BNP_score, by = "Sample_Sentrix_ID")

table(is.na(d1$GDF_score)) # 2954
table(is.na(d1$BNP_score)) # 2808

# Adjust for batch as random effect

library(lme4)
phen <- d1 
phen$Batch <- as.factor(phen$Batch)

mod <- lmer(phen[,145] ~ (1 | Batch), 
                                    na.action = na.exclude, data = phen)
phen[,145] <- resid(mod)

mod2 <- lmer(phen[,146] ~ (1 | Batch), 
                                    na.action = na.exclude, data = phen)
phen[,146] <- resid(mod2)
...
pred <- phen
for(i in names(pred)[145:146]){ 
 pred[,i] <- qnorm((rank(pred[,i],na.last="keep")-0.5)/sum(!is.na(pred[,i])))
}
d1 <- pred

# write.csv(d1, "/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_scores_cox/d1_W4_sample_EPIC.csv", row.names = F)
...
clock <- names(d1)[c(143,144,145,146)] # gdf, bnp, gdf score, bnp score, pQTL gdf, pQTL BNP
...
# Run the below models again and save out per wave 
# Set d1 to each wave file 

# d1 <- W4
d1 <- W4 # 8876 possible W4 values 

# Add W4 cell comps in
cells <- read.delim("/Cluster_Filespace/Marioni_Group/GS/GS_methylation/wave4/wave4_cellcomp.tsv")
cells <- as.data.frame(cells)
names(cells)[1] <- 'Sample_Sentrix_ID'
d1 <- left_join(d1, cells, by = 'Sample_Sentrix_ID')
...
write.csv(comb, "/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_scores_cox/basic_v2.csv", row.names = F)
...
write.csv(comb, "/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_scores_cox/full_v2.csv", row.names = F)
...
for(j in 1:length(clock)){ 
  ...
  cox = cox[cox$age_at_event >=65,]
  mod = coxme(Surv(cox$tte, cox$Event) ~ scale(cox[,clock[[j]]]) + factor(cox$sex.x) + cox$age.x + cox$units + cox$smokingScore + cox$rank + cox$years + cox$bmi + cox$Gran + cox$NK + cox$CD8T + cox$CD4T + cox$Bcell + (1|cox$Sample_Name), varlist = kin_model*2)
  print(clock[[j]])
  print("DEM")
  output_hazard_DEM[j,1] <- as.character(clock[[j]])
  output_hazard_DEM[j,2] <- as.character("Dementia")
...
# comb <- comb[order(comb$`P.Value`),]
# Write ordered basic results 
write.csv(comb, "/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_scores_cox/WBC_v2.csv", row.names = F)
